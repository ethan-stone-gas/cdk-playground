...
// src/telemetry-server.ts
var baseUrl = `http://${process.env.AWS_LAMBDA_RUNTIME_API}/2022-07-01/telemetry`;
var RequestMessageSchema = z.object({
  entityId: z.string(),
  windowIdentifier: z.string(),
  timestamp: z.number()
});
var BodySchema = z.array(
  z.object({
    time: z.string(),
...
var telemetryServer = async (extensionId) => {
  const server = (0, import_http.createServer)((req, res2) => {
    if (req.method === "POST") {
      let body = "";
      req.on("data", (chunk) => {
        body += chunk;
...
// src/rate-limit-extension.ts
var baseUrl2 = `http://${process.env.AWS_LAMBDA_RUNTIME_API}/2020-01-01/extension`;
...
async function next(extensionId) {
  const res = await fetch(`${baseUrl2}/event/next`, {
    method: "get",
    headers: {
      "Content-Type": "application/json",
      "Lambda-Extension-Identifier": extensionId
...
async function error(extensionId, phase, err) {
  ...
  const res = await fetch(`${baseUrl2}/${phase}/error`, {
    method: "post",
    body: JSON.stringify({
...
async function main() {
  ...
  while (true) {
    try {
      ...
      log("error", "error", { err });
      await error(extensionId, "next", err);
    }
  }
}
main();
